...
class RetrofitMockTest {
    ...
    fun `EXPECT POST response`() = runTest {
        val expectedRequest = EXPECTED_COMPLETE_REQUEST.copy(method = Method.Post, body = REQUEST_BODY_RAW)
    ...
    fun `EXPECT PUT response`() = runTest {
        val expectedRequest = EXPECTED_COMPLETE_REQUEST.copy(method = Method.Put, body = REQUEST_BODY_RAW)
        netMock.addMock(expectedRequest, EXPECTED_RESPONSE)
    ...
    fun `EXPECT DELETE response`() = runTest {
        val expectedRequest = EXPECTED_COMPLETE_REQUEST.copy(method = Method.Delete, body = REQUEST_BODY_RAW)
        netMock.addMock(expectedRequest, EXPECTED_RESPONSE)
    ...
    fun `EXPECT PATCH response`() = runTest {
        val expectedRequest = EXPECTED_COMPLETE_REQUEST.copy(method = Method.Patch, body = REQUEST_BODY_RAW)
        netMock.addMock(expectedRequest, EXPECTED_RESPONSE)
    ...
    private companion object {
        ...
        val EXPECTED_RESPONSE = NetMockResponse(code = 201, containsHeaders = mapOf("x" to "y"), body = RESPONSE_BODY)
    }
}